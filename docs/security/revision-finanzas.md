...
| 4 | Los extractos subidos no se persisten en ningún almacenamiento | `test -d`/`test -f` de los tres caminos (los dos directorios más `finance-imports.server.ts`, que es quien recibe de verdad los bytes) + `grep -rn "writeFile\|createWriteStream\|putObject\|storage"` sobre ellos, sin tests | **Verde.** Los tres caminos existen y el grep sale **vacío**, incluido `apps/web/src/lib/server/finance-imports.server.ts` (el fichero que recibe los bytes del multipart y los parsea; antes quedaba fuera del alcance grepeado aunque el «Resultado» ya concluía sobre él): el extracto se parsea en memoria dentro de la transacción autorizada (spec §10). |
...
| 8 | Los logs no imprimen importes ni conceptos | `grep -rn "log\.\(info\|warn\|error\)"` sobre los tres módulos de servidor de finanzas, el paquete `packages/server/src/finance`, el despachador de comandos y los propios endpoints REST (`apps/web/src/routes/api/v1/finance`); más `grep -rn "console\."` | **Verde.** Cuatro llamadas, las cuatro estructuradas y sin carga útil: `finance-access.server.ts:33` → `log.error('finance access check unavailable', { code: errorCode(cause) })`; `finance.server.ts:580` → `log.error('finance api unavailable', { code: errorCode(cause) })`; `imports/preview/+server.ts:49` e `imports/confirm/+server.ts:119` → ambas `log.error('finance import unavailable', { code: errorCode(cause) })` (justo la superficie que antes quedaba fuera del grep, y la que más fácil sería usar para filtrar el nombre del extracto subido). `errorCode` (`packages/server/src/logging.ts:94-103`) devuelve un código de ≤ 64 caracteres o el nombre del error, nunca el mensaje ni un nombre de fichero. Cero `console.*` en el módulo. Las otras tres coincidencias son prosa dentro de comentarios. |
...
`0073 0100 5100 0000 0001`), tarjetas enmascaradas (`XXXX-XXXXX-91009` y
`5402XXXX1111`) e identificador de acreedor SEPA con la **forma** del real y todo ceros
...
# 4 · Los extractos no se persisten (los tres caminos que tocan los bytes)
test -d apps/web/src/routes/api/v1/finance || echo 'CONTROL ROTO: no existe el directorio'
test -d packages/server/src/finance || echo 'CONTROL ROTO: no existe el directorio'
test -f apps/web/src/lib/server/finance-imports.server.ts || echo 'CONTROL ROTO: no existe el fichero'
grep -rn "writeFile\|createWriteStream\|putObject\|storage" \
  apps/web/src/routes/api/v1/finance packages/server/src/finance \
  apps/web/src/lib/server/finance-imports.server.ts \
...
# 8 · Los logs no llevan importes ni conceptos (incluidos los endpoints de importación)
grep -rn "log\.\(info\|warn\|error\)" \
  ...
  apps/web/src/routes/api/v1/finance

# 9 y 10 · Estado real de la base (las tres consultas inlineadas, nada que ensamblar)
...
SELECT rolname, rolbypassrls, rolsuper FROM pg_roles
 WHERE rolname IN ('casa_clara_app', 'casa_clara_app_login', 'casa_clara_worker');
SQL
```
...
- **«Admin sin concesión» no está en la batería de maqueta**, y también es una
  ...
  Lo que sí cubre la maqueta (`apps/web/e2e/finanzas.e2e.ts:292-316`): las siete
