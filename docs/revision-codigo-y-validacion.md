...
| AC-26 | **Parcial** | El criterio se reescribió el 10/08/2026 (ver la nota al pie de `docs/acceptance/brief-v2-adapted.md`): el histórico pasa a estar permitido y lo prohibido es el indicador de cumplimiento. Evidencia: `apps/web/tests/food.integration.test.ts` → `it('loadRoutines: próxima fecha y ocurrencia vigente, sin porcentajes ni histórico (AC-26)')` cierra la superficie del DTO, y `apps/web/tests/calendar-no-metrics.test.ts` vigila vocabulario, forma, color y **geometría** en calendario, Rutinas y Hoy. El hueco que señalaba esta fila —el e2e de maqueta afirmaba que `.routine-progress` *es visible*— está cerrado: la barra con su porcentaje y el anillo de Hoy se retiraron al integrar, y `apps/web/e2e/food.e2e.ts` afirma ahora lo contrario (cuenta sí, `<progress>` y `%` no). **Hueco que queda:** falta la "búsqueda de API/esquema" (ningún test recorre `information_schema` ni la superficie de comandos). |
