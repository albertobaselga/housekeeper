...
INSERT INTO app.routines (id, household_id, title, details, audience, frequency, interval_count, next_due_on, created_by_membership_id) VALUES
  ('${ROUTINE_TODAY}', '${FIXTURE_HOUSEHOLD}', 'Sacar la basura (off)', 'Contenedor amarillo', 'employee', 'daily', 1, current_date, '${ADMIN_MEMBERSHIP}'),
  ('${ROUTINE_LATE}', '${FIXTURE_HOUSEHOLD}', 'Cambiar sábanas (off)', '', 'all', 'weekly', 1, current_date - 2, '${ADMIN_MEMBERSHIP}'),
  ('${ROUTINE_FUTURE}', '${FIXTURE_HOUSEHOLD}', 'Limpiar filtros (off)', '', 'all', 'monthly', 1, current_date + 5, '${ADMIN_MEMBERSHIP}'),
  ('${ROUTINE_FAMILY}', '${FIXTURE_HOUSEHOLD}', 'Revisar seguros (off)', '', 'family', 'quarterly', 1, current_date, '${ADMIN_MEMBERSHIP}');
