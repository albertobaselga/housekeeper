...
// La empleada de la fixture (Ana) ya trae acuerdo con dos versiones. A ella se
// le suman una segunda empleada con contrato propio y una tercera que ya se
// fue: es el escenario que el propietario pidió ver de un vistazo.
...
INSERT INTO app.user_profiles (user_id, display_name, must_change_password) VALUES
  ('fixture:roble:segunda', 'Fixture Segunda Empleada', true),
...
describe.runIf(Boolean(adminUrl))('personal del hogar leído desde Postgres bajo RLS', () => {
  ...
  it('la administración ve a las dos empleadas de hoy y a la de antes', async () => {
    const staff = await loadStaffOverview(ADMIN_USER, FIXTURE_HOUSEHOLD, appPool);
    expect(staff).not.toBeNull();

    // Dos empleadas y el apoyo de la fixture; la que se fue, en el historial.
    expect(staff!.current.map((person) => person.name).sort()).toEqual([
      'Fixture Apoyo Roble',
      'Fixture Empleada Roble',
      'Fixture Segunda Empleada'
    ]);
