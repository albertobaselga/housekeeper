...
// siendo el selector demo, pero la wiki lee y escribe datos reales bajo RLS.
test.skip(!process.env.E2E_DATABASE_URL, 'Requiere E2E_DATABASE_URL (usa pnpm test:e2e:db)');

test('Marta edita en el editor visual y persiste Markdown canónico (revisión 2)', async ({ page }) => {
  ...
  // La pestaña Markdown enseña el MISMO estado, ya serializado a Markdown.
  ...
  // De vuelta a Visual, el editor rehidrata el Markdown canónico y se guarda.
  await page.getByRole('tab', { name: 'Visual' }).click();
  await expect(surface).toBeVisible();
  await expect(surface).toContainText('Programa nuevo');
  await page.getByLabel('Resumen del cambio').fill('Probando el editor visual');
  await page.getByRole('button', { name: 'Guardar cambios' }).click();

  // El envelope edit con baseRevision=1 crea la revisión 2 y la página nueva.
  ...
  await expect(page.locator('.wiki-history summary')).toContainText('2 revisiones');
  await expect(page.locator('.wiki-revisions')).toContainText('r2');
});
