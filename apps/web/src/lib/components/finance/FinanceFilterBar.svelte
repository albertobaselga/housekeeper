  ...
  {#if accountsOpen}
    <nav class="chip-strip finance-accounts" aria-label="Filtrar por cuenta">
      {#each accounts as account (account.id)}
        ...
        <!-- Nota: con `accountIds` vacío («todas») los chips salen todos
             activos por diseño; pulsar uno no lo «despulsa», lo convierte en
             la única cuenta seleccionada. Es el comportamiento portado del
             original, no un bug — pero si el e2e de la T13/T14 espera lo
             contrario, es aquí donde mirar. -->
