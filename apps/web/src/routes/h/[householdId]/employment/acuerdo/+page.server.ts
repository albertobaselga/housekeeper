...
export const actions: Actions = {
  ...
  /**
   * Alta de una persona nueva SIN salir del contrato: el mismo
   * `hireHouseholdMember()` que Personal (identidad + acceso + contrato en un
   * acto, con contraseña provisional), calcado y no reinterpretado. La ruta ya
   * exige `agreement.write` y el servidor vuelve a comprobar la membresía
   * real bajo RLS, que es la comprobación que cuenta.
   */
  hire: async ({ locals, params, request }) => {
    ...
    if (form.get('withAgreement') === 'on') {
      ...
      const parsed = agreementTermsInputSchema.safeParse({
        ...
        // en esta misma pantalla, apilando una versión. Aquí, lo básico.
        extraWorkTypes: [],
        supplements: []
      });
      if (!parsed.success) {
