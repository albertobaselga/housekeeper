import { loadEmploymentOverview } from '$lib/server/employment.server';
import { demoOrUnavailable } from '$lib/server/data-source.server';
import type { PageServerLoad } from './$types';

export const load: PageServerLoad = async ({ locals, params, url, depends }) => {
  // Mismo token que el resumen: registrar o decidir un concepto invalida
  // 'cc:employment' y las dos pestañas se refrescan del mismo load.
  depends('cc:employment');
  const selectedAgreementId = url.searchParams.get('empleada');
  const overview = locals.user
    ? await loadEmploymentOverview(
        { id: locals.user.id },
        params.householdId,
        undefined,
        undefined,
        selectedAgreementId
      )
    : null;
  if (overview) return { overview };
  // La maqueta de demostración vive solo en el Resumen: aquí, sin base de
  // datos, la página dice su estado vacío en vez de fingir formularios que no
  // escribirían en ninguna parte.
  return demoOrUnavailable(() => ({ overview: null }));
};
